...
I end up with `r nrow(train)` datapoints in my training data and `r nrow(test)` in my testing data. 

Then I need to build a model for prediction. I have chosen an ordered logit to test first because my data is not purely categorical, but has a logical order to it. A non-grade crime (like a DUI) is less serious than a misdemeanor (like a retail theft, for example), which is less serious than a felony (like an aggravated assault causing serious bodily injury), which is less serious than murder (which receives an automatic life sentence in Pennsylvania). 
